	...
	if (openArg.has_value())
	{
		...
		if (Platform::FileExists(openArg.value()))
		{
			try
			{
				...
				{
					new ErrorMessage(ByteString("錯誤").FromUtf8(), ByteString("無法讀取檔案").FromUtf8());
				}
				else
				{
			...
			{
				new ErrorMessage(ByteString("錯誤").FromUtf8(), ByteString("無法載入沙盤檔案:\n").FromUtf8() + ByteString(e.what()).FromUtf8()) ;
			}
		}
		else
		{
			new ErrorMessage(ByteString("錯誤").FromUtf8(), ByteString("無法載入此檔案").FromUtf8());
		}
	}
