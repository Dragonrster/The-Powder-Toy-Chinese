	...
	mousePosition(0, 0)
{
	...
	reloadButton = new ui::Button(ui::Point(currentX, Size.Y-16), ui::Point(17, 15), "", ByteString("重新加载沙盘").FromUtf8());
	...
	tagSimulationButton = new ui::Button(ui::Point(currentX, Size.Y-16), ui::Point(WINDOWW - 402, 15), ByteString("[无标签]").FromUtf8(), ByteString("添加沙盘标签").FromUtf8());
	...
	simulationOptionButton = new ui::Button(ui::Point(Size.X-48, Size.Y-16), ui::Point(15, 15), "", ByteString("设置").FromUtf8());
	...
	pauseButton = new ui::Button(ui::Point(Size.X-16, Size.Y-16), ui::Point(15, 15), "", ByteString("暂停/继续").FromUtf8());  //Pause
	pauseButton->SetIcon(IconPause);
	pauseButton->SetTogglable(true);
	pauseButton->SetActionCallback({ [this] { c->SetPaused(pauseButton->GetToggleState()); } });
	AddComponent(pauseButton);

	ui::Button * tempButton = new ui::Button(ui::Point(WINDOWW-16, WINDOWH-32), ui::Point(15, 15), 0xE065, ByteString("查找元素").FromUtf8());
	tempButton->Appearance.Margin = ui::Border(0, 2, 3, 2);
	tempButton->SetActionCallback({ [this] { c->OpenElementSearch(); } });
	AddComponent(tempButton);

	colourPicker = new ui::Button(ui::Point((XRES/2)-8, YRES+1), ui::Point(16, 16), "", ByteString("选择颜色").FromUtf8());
...
void GameView::NotifyColourPresetsChanged(GameModel * sender)
{
	...
	{
		ToolButton * tempButton = new ToolButton(ui::Point(currentX, YRES+1), ui::Point(30, 18), "", "", ByteString("预设装饰").FromUtf8());
...
void GameView::NotifySaveChanged(GameModel * sender)
{
	...
	if (sender->GetSave())
	{
		...
		if (sender->GetSave()->GetID())
		{
			...
			{
				tagSimulationButton->SetText(ByteString("无标签").FromUtf8());
			}
		}
		else
		{
	...
	{
		saveSimulationButton->SetShowSplit(false);
		saveSimulationButton->SetText(ByteString("[未命名沙盘]").FromUtf8());
...
void GameView::OnKeyPress(int key, int scan, bool repeat, bool shift, bool ctrl, bool alt)
{
	...
	case SDL_SCANCODE_C:
		if(ctrl)
		{
			...
			selectPoint1 = selectPoint2 = ui::Point(-1, -1);
			isMouseDown = false;
			buttonTip = ByteString("\x0F\xEF\xEF\020单击并拖动以指定要复制的区域(右键单击=取消)").FromUtf8();
			buttonTipShow = 120;
		}
		break;
	case SDL_SCANCODE_X:
		if(ctrl)
		{
			...
			selectPoint1 = selectPoint2 = ui::Point(-1, -1);
			isMouseDown = false;
			buttonTip = ByteString("\x0F\xEF\xEF\020单击并拖动以指定要剪切复制的区域(右键单击=取消)").FromUtf8();
			buttonTipShow = 120;
		}
		break;
	case SDL_SCANCODE_V:
...
void GameView::DoDraw()
{
	...
	{
		g->BlendLine({ x, YRES + 1 }, { x, YRES + 18 }, 0x0000ff_rgb .WithAlpha(fadeout[x]));
		g->BlendLine({ XRES - x, YRES + 1 }, { XRES - x, YRES + 18 }, 0x0000ff_rgb .WithAlpha(fadeout[x]));
	}
...
void GameView::SetSaveButtonTooltips()
{
	if (!Client::Ref().GetAuthUser())
		...
		saveSimulationButton->SetToolTips(ByteString("重新上传当前沙盘").FromUtf8(), ByteString("上传一个新沙盘,按住Ctrl保存到本地").FromUtf8());
}
...
void GameView::OnDraw()
{
	...
	if(selectMode!=SelectNone)
	{
		...
		else
		{
			if(selectPoint1.X==-1)
			{
				g->BlendFilledRect(RectSized(Vec2{ 0, 0 }, Vec2{ XRES, YRES }), 0x0000ff_rgb .WithAlpha(100));
			}
			else
			{
				int x2 = (selectPoint1.X>selectPoint2.X)?selectPoint1.X:selectPoint2.X;
				...
				g->BlendFilledRect(RectSized(Vec2{ 0, 0 }, Vec2{ XRES, y1 }), 0x0000ff_rgb .WithAlpha(100));
				g->BlendFilledRect(RectSized(Vec2{ 0, y2+1 }, Vec2{ XRES, YRES-y2-1 }), 0x0000ff_rgb .WithAlpha(100));

				g->BlendFilledRect(RectSized(Vec2{ 0, y1 }, Vec2{ x1, (y2-y1)+1 }), 0x0000ff_rgb .WithAlpha(100));
				g->BlendFilledRect(RectSized(Vec2{ x2+1, y1 }, Vec2{ XRES-x2-1, (y2-y1)+1 }), 0x0000ff_rgb .WithAlpha(100));
	...
	if (logEntries.size())
	{
		...
		for(iter = logEntries.begin(); iter != logEntries.end(); iter++)
		{
			...
			startY -= 14;
			g->BlendFilledRect(RectSized(Vec2{ startX-3, startY-3 }, Vec2{ Graphics::TextSize(message).X + 5, 14 }), 0x0000ff_rgb .WithAlpha(std::min(100, alpha)));
	...
	if (recording)
	{
		...
		g->BlendFilledRect(RectSized(Vec2{ XRES-20-textWidth, 12 }, Vec2{ textWidth+8, 15 }), 0x0000ff_rgb .WithAlpha(127));
	...
	else if(showHud)
	{
		...
		g->BlendFilledRect(RectSized(Vec2{ XRES-20-textWidth, 12 }, Vec2{ textWidth+8, 15 }), 0x0000ff_rgb .WithAlpha(int(alpha*0.5f)));
		...
		if (showDebug)
		{
			...
			g->BlendFilledRect(RectSized(Vec2{ XRES-20-textWidth, 27 }, Vec2{ textWidth+8, 14 }), 0x0000ff_rgb .WithAlpha(int(alpha*0.5f)));
	...
	if(showHud && introText < 51)
	{
		...
		if (rendererSettings->gridSize)
			fpsInfo <<  ByteString(" [网格: ").FromUtf8()<< rendererSettings->gridSize <<  ByteString("]").FromUtf8();
		if (rendererSettings->findingElement)
		...
		int alpha = 255-introText*5;
		g->BlendFilledRect(RectSized(Vec2{ 12, 12 }, Vec2{ textWidth+8, 15 }), 0x0000ff_rgb .WithAlpha(int(alpha*0.5)));
	...
	{
		g->BlendFilledRect(RectSized(Vec2{ 0, 0 }, WINDOW), 0x0000ff_rgb .WithAlpha(introText>51?102:introText*2));
